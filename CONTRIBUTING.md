...
Thanks for considering a contribution. Parakh aims to stay small, dependency-free
at its core, and easy to drop into a pipeline.

## Setup

```bash
git clone https://github.com/sarcascoder/parakh
cd parakh
pip install -e ".[dev]"
pytest -q
```
...
1. **Core stays stdlib-only.** Anything in `parakh/` (except optional extractor
...
3. **Be honest in docs.** Parakh is a focused library, not an IDP platform. Don't
